...
- 구현 전에 가정, 성공 기준, 불확실한 해석을 명시한다. 모호하면 사용자에게 묻는다.
- 최소 변경을 우선한다. 요청되지 않은 기능, 추상화, 리팩터링, 설정 가능성은 추가하지 않는다.
- 변경한 줄은 사용자 요청 또는 Codex 피드백에 직접 연결되어야 한다.
- 완료 주장 전 관련 검증을 실행하고 핵심 출력과 exit code를 확인한다. 코드 변경 후에는 `git diff` 또는 `rg`로 의도한 변경도 확인한다.
- 위험·비가역 동작(삭제, force push, 히스토리 재작성, 안전장치 우회, 파괴적 DB 작업, 외부 발신/deploy, `sudo`, 프로젝트 범위 밖 쓰기)은 사용자 명시 승인 없이 실행하지 않는다.
- Codex 피드백은 검토 후 수용/기각한다. 잘못된 지적은 적용하지 말고 이유를 기록한다.
...
작자 라운드에서는 먼저 task를 검증 가능한 목표로 바꾸고, 가장 작은 변경으로 구현한다. 구현 뒤에는 가능한 관련 테스트/빌드/린트/grep 중 하나 이상을 실행해 결과를 확인한다. 검증 명령이 모호하면 README, Makefile, package.json, go.mod 등 프로젝트 표준을 먼저 확인한다.
